...
struct AgentConfig {
    ...
    std::string model_id = "llama-3.1-8b-instant";
...
private:
    ...
    json serialize_entry(const SemanticMemoryEntry& entry) const {
        json j;
    ...
    SemanticMemoryEntry deserialize_entry(const json& item) const {
        ...
        if(fs::exists(memory_file)) {
            try {
                ...
                for(const auto& item : data) {
                    memory_store.push_back(deserialize_entry(item));
                }
    ...
    void save_memory() {
        try {
            ...
            for(const auto& entry : memory_store) {
                data.push_back(serialize_entry(entry));
            }
...
public:
    AnalysisResult analyze_session(const std::string& task, bool success, const std::string& observations, int turn_count) {
        ...
        if (!success) {
            ...
            if (obs_lower.find("segmentation fault") != std::string::npos || 
                obs_lower.find("return code: 139") != std::string::npos) {
                ...
                std::cout << "[ReflectionEngine] SEGMENTATION FAULT DETECTED (turn " << turn_count 
                          << ") - Suggested fix pattern: add null check OR use std::unique_ptr / bounds checking\n";
            }
            else if (obs_lower.find("timeout") != std::string::npos || turn_count >= 9) {
...
private:
    ...
    std::unordered_map<std::string, std::function<std::string(const json&)>> tool_handlers_;
    void register_tools();
    bool is_debug_task = false;
    bool debug_must_read_log_next_turn = false;
    int  debug_crash_count = 0;
};
...
void CodingAgent::register_tools() {
    tool_handlers_["list_directory_files"] = [this](const json& /*args*/) { return list_directory_files(); };
    tool_handlers_["read_file"] = [this](const json& args) { return read_file(args["file_path"]); };
    tool_handlers_["write_file"] = [this](const json& args) { return write_file(args["file_path"], args["content"]); };
    tool_handlers_["make_directory"] = [this](const json& args) { return make_directory(args["dir_path"]); };
    tool_handlers_["execute_file"] = [this](const json& args) { return execute_file(args["file_path"]); };
    tool_handlers_["undo_last_change"] = [this](const json& args) { return undo_last_change(args["file_path"]); };
    tool_handlers_["run_code"] = [this](const json& args) {
        std::string lang = args.value("language", "auto");
        if (args.contains("python_code")) {
            return run_code(args["python_code"], "python");
        } else if (args.contains("code")) {
            return run_code(args["code"], lang);
        } else {
            return run_code("", lang);
        }
    };
    tool_handlers_["install_package"] = [this](const json& args) { return install_package(args["package"]); };
    tool_handlers_["search_in_files"] = [this](const json& args) {
        std::vector<std::string> exts;
        if (args.contains("include_extensions") && args["include_extensions"].is_array()) {
            for (const auto& e : args["include_extensions"]) {
                exts.push_back(e.get<std::string>());
            }
        }
        return search_in_files(args["pattern"], exts);
    };
    tool_handlers_["get_environment_info"] = [this](const json& /*args*/) { return get_environment_info(); };
    tool_handlers_["clear_cache"] = [this](const json& /*args*/) { return clear_cache(); };
}
...
CRITICAL RULES FOR DEBUGGING TASK 14:
- This is the Automated Debugging via Log Analysis task.
- After any rc=139 / Segmentation fault, the VERY NEXT turn must start with read_file("execution_traceback.log").
- Do NOT combine reading the log and fixing the code in the same actions block.
- Only declare outcome: success after you have seen (1) a crash in the log, (2) you read the log, (3) a clean rc=0 run of the fixed version.
- Always use the filename "memory_leak.cpp".
...
std::string CodingAgent::parse_and_run_actions(const std::string& text) {
    ...
    for (const auto& raw_block : matches) {
		...
		if (is_debug_task) {
            bool has_read_log = false;
            bool has_fix = false;
            bool has_crash_in_prev = last_observations.find("139") != std::string::npos ||
                                     last_observations.find("Segmentation fault") != std::string::npos;
            if (debug_must_read_log_next_turn && !has_read_log) {
                std::cout << "[DEBUG GUARD] FORCED LOG READ REQUIRED - previous turn had rc=139\n";
                return "Observation: CRITICAL RULE VIOLATION - You MUST start this turn with read_file(\"execution_traceback.log\"). "
                       "No other actions are allowed until the log is read.";
            }
            for (const auto& action : actions) {
                std::string tool = action.value("tool", "");
                std::string filepath = action["args"].value("file_path", "");
                if (tool == "read_file" && filepath == "execution_traceback.log")
                    has_read_log = true;
                if ((tool == "write_file" || tool == "undo_last_change") &&
                    filepath.find("memory_leak") != std::string::npos)
                    has_fix = true;
            }
            if (has_read_log && has_fix) {
                std::cout << "[DEBUG GUARD] BLOCKED: read_log + fix in SAME turn\n";
                return "Observation: CRITICAL RULE VIOLATION - You read the log AND tried to fix in the same turn. "
                       "This is forbidden. You must wait until the NEXT turn to apply any fix.";
            }
            if (has_crash_in_prev && !has_read_log) {
                std::cout << "[DEBUG GUARD] MISSING LOG READ after crash - forcing read_file\n";
                debug_must_read_log_next_turn = true;   // NEW: Persist the requirement
                return "Observation: CRITICAL RULE VIOLATION - Previous execution had a segmentation fault (rc=139). "
                       "The VERY NEXT action MUST be read_file(\"execution_traceback.log\"). "
                       "You skipped it. Fix this immediately.";
            }
            if (has_read_log) {
                debug_must_read_log_next_turn = false;
                std::cout << "[DEBUG GUARD] Log read acknowledged - fix allowed next turn\n";
            }
        }
        ...
        for (auto& action : actions) {
            ...
            if (!action.contains("tool") || !action.contains("args"))
                continue;
            std::string original_tool_name = action["tool"];
            std::string tool_name = original_tool_name;
            ...
            if (tool_name == "execute_python_file") tool_name = "execute_file";
            ...
            block_results.push_back("Tool " + original_tool_name + " Result: " + result);
        }
...
bool CodingAgent::check_outcome(const std::string& text) {
    ...
    if (is_debug_task) {
        bool had_crash = last_observations.find("139") != std::string::npos ||
                         last_observations.find("Segmentation fault") != std::string::npos;
        bool had_clean_run = last_observations.find("Return code: 0") != std::string::npos;
        if (had_crash && !had_clean_run) {
            std::cout << "[Debug Guard] Blocking success: you must show a clean run AFTER fixing the bug\n";
            return false;
        }
    } else {
...
std::vector<std::string> CodingAgent::prune_context(const std::vector<std::string>& context) {
    if((int)context.size() <= 4) return context;  // reduce from 6 to 4 for safety
    std::vector<std::string> pruned;
    pruned.push_back(context[0]);  // keep system/task intro
    for(size_t i = context.size() - 3; i < context.size(); ++i)  // keep only last 3 turns
...
void CodingAgent::reflect(const std::string& task, bool success, const std::string& observations, int turn_count) {
    ...
    if (!success && analysis.error_type == "segmentation_fault") {
        std::cout << "[Reflection] Detected segmentation_fault - Suggestion: memory_safety_checker\n";
    }
}
...
std::string CodingAgent::run(const std::string& task_description, int max_turns) {
    ...
    if (cached_opt.has_value()) {
        ...
        if (observations.length() > 3500) {
            std::string clean = observations.substr(0, 800);
            clean += "\n\n[CRASH/GARBAGE OUTPUT TRUNCATED - Segfault or buffer overflow detected. "
                     "Full noisy memory dump removed to stay under token limits.]";
            observations = clean;
            last_observations = observations;
        }
    ...
    is_debug_task = task_description.find("Automated Debugging via Log Analysis") != std::string::npos ||
                    task_description.find("memory_leak") != std::string::npos ||
                    task_description.find("segmentation fault") != std::string::npos;
    if (is_debug_task) {
        executor.clear_traceback_log();
        std::cout << "[DEBUG TASK] Cleared execution_traceback.log for clean run\n";
        std::string starter_bug = R"cpp(#include <iostream>
int main() {
    int* p = nullptr;
    *p = 42;  // guaranteed segmentation fault
    std::cout << "This line should never execute" << std::endl;
    return 0;
})cpp";
        file_manager.write_file("memory_leak.cpp", starter_bug);
        std::cout << "[DEBUG TASK] Pre-created guaranteed crashing memory_leak.cpp\n";
        debug_must_read_log_next_turn = false;
        debug_crash_count = 0;
        std::cout << "[DEBUG TASK] Debug state reset - ready for crash → log → fix sequence\n";
    }
    ...
    for (int turn = 0; turn < max_turns; turn++) {
        ...
        if (!success_declared && is_debug_task &&
            (last_observations.find("139") != std::string::npos ||
             last_observations.find("Segmentation fault") != std::string::npos)) {
            debug_crash_count++;
            debug_must_read_log_next_turn = true;
            session_context.push_back(
                "REFLECTIONENGINE ALERT: Segmentation fault detected (crash #" + std::to_string(debug_crash_count) + "). "
                "YOU MUST follow the EXACT CRITICAL SEQUENCE: "
                "1. read_file(\"execution_traceback.log\") as the VERY FIRST action next turn. "
                "2. ONLY THEN fix the code in the following turn. "
                "Do NOT combine read + fix. Do NOT declare success until you have a clean rc=0 run.");
        }
...
int main() {
    ...
    try {
        ...
        if(choice == "2") {
            ...
            if(task.empty()) {
                task = R"(Task 14. Automated Debugging via Log Analysis: Provide a C++ source file that contains a subtle memory leak 
                or a segmentation fault under specific input conditions. The agent must execute the file, capture the failure in 
                $execution\_traceback.log$, and use its ReflectionEngine to identify the bottleneck or error type. It should then attempt 
                a fix using the $undo\_last\_change$ or $write\_file$ tools to iterate toward a stable solution. The agent must strictly 
                follow the CRITICAL SEQUENCE in its own system prompt.)";                
            }
            std::cout << "Max turns (default 6): ";
            std::string turns_str;
            std::getline(std::cin, turns_str);
            bool is_debug = task.find("Automated Debugging via Log Analysis") != std::string::npos ||
                            task.find("memory_leak") != std::string::npos ||
                            task.find("segmentation fault") != std::string::npos;
            int max_turns = turns_str.empty() ? (is_debug ? 12 : 6) : std::stoi(turns_str);
...
Task 11. Multi-Language Data Bridge: Create a C++ program named $generator.cpp$ that simulates $10^6$ particle interactions and 
saves the state to a $data.json$ file using the $nlohmann/json$ library. Subsequently, create a Python script $analyzer.py$ that 
reads this JSON file, calculates the mean energy of the particles, and generates a summary report. This tests the agent ability 
to manage dependencies across different languages and ensure data consistency between processes.

Task 12. Cross-File Refactoring: Task the agent with renaming a specific class or utility function in a multi-file project. For 
example, it must create a directory $src/math$, move several mathematical functions from a single $utils.py$ into separate modules, 
and then update the import statements in an $app.py$ located in the root directory. This evaluates the efficiency of the $FileManager$ 
in handling path updates and recursive directory searches.

Task 13. Secure Input Validation: Task the agent with creating a Python application that accepts user input to query an SQLite 
database. The agent must implement a defense against SQL injection by using parameterized queries. Then, attempt to "attack" its 
own script with a common injection string to verify that the validation logic holds. This tests higher-order reasoning regarding 
security and defensive programming.

Task 14. Automated Debugging via Log Analysis: Provide a C++ source file that contains a subtle memory leak or a segmentation fault 
under specific input conditions. The agent must execute the file, capture the failure in $execution\_traceback.log$, and use its 
ReflectionEngine to identify the bottleneck or error type. It should then attempt a fix using the $undo\_last\_change$ or 
$write\_file$ tools to iterate toward a stable solution. The agent must strictly follow the CRITICAL SEQUENCE in its own system prompt.

Task 15. Networked Resource Fetching: Require the agent to use the $curl$ capability to interface with a public API to retrieve 
current weather or financial data. The agent must then use $openssl/md5$ to create a unique checksum of the response to verify 
data integrity before processing. This evaluates the ability to handle external libraries and asynchronous-like behavior within 
the execution environment.

Task 16. Monte Carlo Integration Create a c++ program monte_carlo.cpp to estimate ∫sin(x)dx from 0 to π 
using 10,000,000 random points. Compare estimate to analytical value 2. Print absolute error.
