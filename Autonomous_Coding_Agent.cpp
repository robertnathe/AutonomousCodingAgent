...
string md5_hash(const string& s) {
    ...
    ostringstream oss;
    for (unsigned int i = 0; i < 4 && i < digest_len; ++i)
...
string error_type_from_output(const string& output) {
    ...
    if (lo.find("return code: -9") != string::npos) return "timeout";
    return "unknown";
}
...
class SemanticMemoryManager {
    ...
    int max_entries;
    fs::path db_path;
    list<FailurePattern> failure_patterns;
...
public:
    ...
    struct JsonTokenScanner {
        const string& text;
        size_t pos;
        JsonTokenScanner(const string& t) : text(t), pos(0) {}
        enum TokenType { NONE, LBRACE, RBRACE, LBRACKET, RBRACKET, 
                         STRING, COLON, COMMA, LITERAL, WHITESPACE, INVALID };
        struct Token {
            TokenType type;
            size_t start;
            size_t end;
        ...
        Token next() {
            if (pos >= text.size()) return {NONE, pos, pos, ""};  
            size_t start = pos;
            char c = text[pos];
            if (isspace(static_cast<unsigned char>(c))) {
                while (pos < text.size() && isspace(static_cast<unsigned char>(text[pos]))) pos++;
                return {WHITESPACE, start, pos, text.substr(start, pos - start)};
            }
            switch (c) {
                case '{': pos++; return {LBRACE, start, pos, "{"};
                case '}': pos++; return {RBRACE, start, pos, "}"};
                case '[': pos++; return {LBRACKET, start, pos, "["};
                case ']': pos++; return {RBRACKET, start, pos, "]"};
                case ':': pos++; return {COLON, start, pos, ":"};
                case ',': pos++; return {COMMA, start, pos, ","};
            }
            if (c == '"') {
                pos++;
                string value;
                ...
                while (pos < text.size()) {
                    ...
                    if (escaped) {
                        ...
                        pos++;
                        return {STRING, start, pos, value};
                    } else {
                        value += ch;
                    }
                    pos++;
                }
                return {INVALID, start, pos, text.substr(start, pos - start)};
            }
            if (isdigit(static_cast<unsigned char>(c)) || c == '-' || c == '+' || c == '.' ||
                (pos + 4 <= text.size() && text.substr(pos, 4) == "true") ||
                (pos + 5 <= text.size() && text.substr(pos, 5) == "false") ||
                (pos + 4 <= text.size() && text.substr(pos, 4) == "null")) {
                ...
                return {LITERAL, start, pos, text.substr(start, pos - start)};
            }
            pos++;
            return {INVALID, start, pos, string(1, c)};
        }
        void reset(size_t p = 0) { pos = p; }
    };
    vector<pair<size_t, size_t>> find_json_objects_token_aware(const string& text) {
        vector<pair<size_t, size_t>> objects;  
        size_t pos = 0;
        while (pos < text.size()) {
            while (pos < text.size() && text[pos] != '{') pos++;
            if (pos >= text.size()) break;
            size_t obj_start = pos;
            JsonTokenScanner scanner(text);
            scanner.pos = pos;
            auto first_tok = scanner.next();
            if (first_tok.type != JsonTokenScanner::LBRACE) {
                pos++;
                continue;
            }
            int depth = 1;
            bool valid = true;
            while (depth > 0 && scanner.pos < text.size()) {
                auto tok = scanner.next();
                if (tok.type == JsonTokenScanner::NONE) {
                    valid = false;
                    break;
                }
                if (tok.type == JsonTokenScanner::LBRACE) depth++;
                else if (tok.type == JsonTokenScanner::RBRACE) depth--;
            }
            if (valid && depth == 0) {
                objects.emplace_back(obj_start, scanner.pos);
                pos = scanner.pos;
            } else {
                pos = obj_start + 1;
            }
    ...
    string sanitize_newlines_token_aware(const string& text) {
        JsonTokenScanner scanner(text);
        string result;
        result.reserve(text.size() * 2);
        while (scanner.pos < text.size()) {
            auto tok = scanner.next();
            if (tok.type == JsonTokenScanner::STRING) {
                result += '"';
                ...
                result += '"';
            } else if (tok.type == JsonTokenScanner::NONE) {
                break;
            } else {
                result += text.substr(tok.start, tok.end - tok.start);
            }
        }
        return result;
    }
    string fix_triple_quotes_in_json_strings(const string& text) {
        JsonTokenScanner scanner(text);
        string result;
        result.reserve(text.size() * 2);
        while (scanner.pos < text.size()) {
            auto tok = scanner.next();
            if (tok.type == JsonTokenScanner::STRING) {
                string fixed = tok.value;
                size_t i = 0;
                string processed;
                while (i < fixed.size()) {
                    if (i + 2 < fixed.size() && fixed[i] == '"' && 
                        fixed[i+1] == '"' && fixed[i+2] == '"') {
                        size_t content_start = i + 3;
                        size_t j = content_start;
                        while (j + 2 < fixed.size()) {
                            if (fixed[j] == '"' && fixed[j+1] == '"' && fixed[j+2] == '"') {
                                string content = fixed.substr(content_start, j - content_start);
                                string escaped;
                                for (char c : content) {
                                    if (c == '"') escaped += "\\\"";
                                    else if (c == '\n') escaped += "\\n";
    ...
    vector<json> deduplicate_actions(const vector<json>& actions) {
        vector<json> unique;
        set<string> seen;
    ...
    vector<json> extract_actions(const string& text) {
        ...
        {
            auto objects = find_json_objects_token_aware(text);
            for (const auto& [start, end] : objects) {
                string snippet = text.substr(start, end - start);
                try {
                    json obj = json::parse(snippet);
                    if (obj.is_object() && obj.contains("tool"))
                        actions.push_back(apply_aliases(obj));
                } catch (const exception& e) {
                    errors.push_back("S1: " + string(e.what()));
                }
            }
            if (!actions.empty()) return deduplicate_actions(actions);
        }
        {
            string sanitized = sanitize_newlines_token_aware(text);
            auto objects = find_json_objects_token_aware(sanitized);
            for (const auto& [start, end] : objects) {
                string snippet = sanitized.substr(start, end - start);
                try {
                    json obj = json::parse(snippet);
                    if (obj.is_object() && obj.contains("tool"))
                        actions.push_back(apply_aliases(obj));
                } catch (const exception& e) {
                    errors.push_back("S2: " + string(e.what()));
                }
            }
            if (!actions.empty()) return deduplicate_actions(actions);
        }
        {
            string fixed = fix_triple_quotes_in_json_strings(text);
            auto objects = find_json_objects_token_aware(fixed);
            for (const auto& [start, end] : objects) {
                string snippet = fixed.substr(start, end - start);
                try {
                    json obj = json::parse(snippet);
                    if (obj.is_object() && obj.contains("tool"))
                        actions.push_back(apply_aliases(obj));
                } catch (const exception& e) {
                    errors.push_back("S3: " + string(e.what()));
                }
            }
            if (!actions.empty()) return deduplicate_actions(actions);
        }
        {
            string combined = sanitize_newlines_token_aware(text);
            combined = fix_triple_quotes_in_json_strings(combined);
            auto objects = find_json_objects_token_aware(combined);
            for (const auto& [start, end] : objects) {
                string snippet = combined.substr(start, end - start);
                try {
                    json obj = json::parse(snippet);
                    if (obj.is_object() && obj.contains("tool"))
                        actions.push_back(apply_aliases(obj));
                } catch (const exception& e) {
                    errors.push_back("S4: " + string(e.what()));
                }
            }
            if (!actions.empty()) return deduplicate_actions(actions);
        }
        {
            ...
            while (pos < text.size()) {
                ...
                for (size_t end = brace + 1; end <= text.size() && end < brace + 10000; ++end) {
                    if (text[end-1] == '}') {
                        string snippet = text.substr(brace, end - brace);
                        vector<string> attempts = {snippet};
                        string s1 = sanitize_newlines_token_aware(snippet);
                        if (s1 != snippet) attempts.push_back(s1);
                        string s2 = fix_triple_quotes_in_json_strings(snippet);
                        if (s2 != snippet) attempts.push_back(s2);
                        string s3 = fix_triple_quotes_in_json_strings(s1);
                        if (s3 != s1 && s3 != snippet) attempts.push_back(s3);
                        for (const auto& attempt : attempts) {
                            try {
                                json obj = json::parse(attempt);
                                if (obj.is_object() && obj.contains("tool")) {
                                    actions.push_back(apply_aliases(obj));
                                    goto next_brace;
                                }
        ...
        if (actions.empty() && !errors.empty()) {
            cerr << "[extract_actions] All strategies failed. Last errors:" << endl;
            for (size_t i = max(0, (int)errors.size() - 5); i < errors.size(); ++i) {
                cerr << "  " << errors[i] << endl;
            }
        }
                ...
                if (tool_name == "write_file") {
                    ...
                    for (const auto& check : validation_checks) {
                        if (check.value("type","") == "execution" && check.value("file","") == fp) {
                            ...
                            bool has_rc      = result.find("Return code:") != string::npos;
							bool rc_ok       = result.find("Return code: 0") != string::npos;
							bool rc_timeout  = result.find("Return code: -9") != string::npos;
							if (has_rc && !rc_ok) {
							...
							if (check.contains("expect")) {
							    ...
							    if (rl.find(el) == string::npos) {
							        if (!rc_ok) {
							            if (rc_timeout) {
							                last_validation_feedback =
							                    "Script " + fp + " TIMED OUT (killed after exceeding time limit). "
							                    "The algorithm may be too slow or contain an infinite loop. "
							                    "Optimize for performance or fix termination conditions.\nActual output:\n" + result.substr(0, 600);
							            } else {
							                last_validation_feedback =
							                    "Script " + fp + " CRASHED or FAILED with non-zero exit code. "
							                    "Fix the runtime error before worrying about output content.\nActual output:\n" + result.substr(0, 600);
							            }
							        } else {
							            last_validation_feedback =
							                "Script " + fp + " ran successfully (exit code 0) but output is MISSING "
							                "the required substring: '" + expect_str + "'. Your output MUST contain this exact text.\nActual output:\n" + result.substr(0, 600);
							        }
